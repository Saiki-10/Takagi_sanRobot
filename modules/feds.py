...
import Takagi_sanRobot.modules.sql.feds_sql as sql
...
from Takagi_sanRobot.modules.helper_funcs.chat_status import is_user_admin
from Takagi_sanRobot.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Takagi_sanRobot.modules.helper_funcs.string_handling import markdown_parser
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id == bot.id:
        message.reply_text(
            "What is funnier than kicking the group creator? Boink??."
        )
        return
    ...
    if user_id == OWNER_ID:
        message.reply_text("Owner cannot be fed banned!")
        return
